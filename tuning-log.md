...
## ultracode 对抗审查评测管线：28 项确认、17 项实质，官方基线戴星号 (2026-06-11)

工作流 `wf_53ebd6b9-c15`（66 agents / 4.39M tok）：5 维猎错（度量语义/匹配链/变异 harness/方法论循环性/prompt-解析契约）→ 每发现双镜头对抗核验（事实反驳默认立场驳倒 + 基线影响量化）→ 完整性批评员。**30 猎获 / 28 确认 / 17 实质影响基线 / 2 驳回**。全文 `outputs/_pipeline_audit/report.md`（原始票据 `audit_raw.json`）。

**两基线裁定**：gold 尺 0.4480 —— 绝对水位被循环回写充气（40% micro TP 是自家回写项、同模型家族单裁判），但 **+0.043 prompt 增益经对称性检验存活**（双臂同 gold，驳回了"增益翻负"指控）；变异尺 0.760 —— 三处折扣：单边重跑（只复查污染 miss 不复查 caught）、0009 m0/m1 注入字节级重复双计、McNemar p≈0.18 不显著；且 0074 m3 系判官废票占位误判（4/75 废票算 no 票），真实 20/25。
