...
- Verbatim text-node audit (content/text items): when an item verifies displayed text or labels, read EVERY text node in the relevant region of the snapshot word by word — including secondary labels, counters, badges, "X of Y" indicators, units/suffixes, and empty-state messages — and check each against the app's actual state/data. Wrong wording, a stale count, or a label contradicting the data is a FAIL even when the "main" element looks right; do not stop at the first matching element.
- State-word cross-check: whenever a visible label or control carries a state word (Yes/No, On/Off, Enabled/Disabled, Active/Inactive, asc/desc, ascending/descending, etc.), explicitly compare that state word against the control's ACTUAL state or effect (the real data order, the aria/disabled attribute, what happens when it is used). A state word that contradicts the real state is a FAIL, even if the item's main action otherwise works.
- Same-control double activation: when an item involves a toggle, sort direction, or any control whose state should flip on each activation, activate the SAME control at least twice in a row and verify the state flips correctly each time (A→B→A). A single activation proves only half the cycle and silently misses inverted or no-op second toggles.
